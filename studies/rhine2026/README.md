...
| response mapping (re-ordered 04 Sep 03:55 after the main result) | control (6 undisrupted weeks) → `--substitution-share 0.7` → `0.3 --critical-input-threshold 0.10` (cascade sensitivity, new flag) → `0.5` → `0.15` (4 recovery weeks) → 2018 at 0.3; outputs `C:\dsc_runs\rhine2026\<name>`, `analysis.txt` written by `EU/runs/postprocess_rhine.ps1` | running |
