...
| response mapping (re-ordered 04 Sep 03:46 after the main result) | control → `--substitution-share 0.7` → `0.3 --critical-input-threshold 0.10` → `0.3 --input-criticality` (IHS Markit survey, `build_criticality_eu.py`) → `0.5` → `0.15` → 2018; outputs `C:\dsc_runs\rhine2026\<name>`, `analysis.txt` per folder | **ceiling 0.70 done 06:45**: corridor firms below 99 % peak 77 % (DIHK 33 %), consumption loss peak 9.7 %, DEU VA loss peak 16.8 %/week, 12.2 % of a quarter cumulated (0.30 run: 45.7 % and 30.2 %), EU 7.5 % of a quarter, fill rate 91 % at the trough, max surcharge 3 %. Roughly linear in the withheld share, so no ceiling matches the macro range without also killing the survey share: the propagation rule is the problem. The threshold-0.10 run was stopped at 07:00 because the threshold sat in the agents cache key (1 h rebuild per run; fixed by re-applying it on load, disrupt-sc next commit) |
