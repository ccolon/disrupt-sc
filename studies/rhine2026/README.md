...
candidate final baseline, to be confirmed by the user; their `--no-pooling` twins are `2026_seed42_rates` /
...
14 Sep probably too. Every 2026 run above is out of date from step 12 on. By the user's condition the
...
1. Weekly Kaub levels (`scenarios/2026.csv`: PEGELONLINE daily means 22 Jun – 4 Sep 2026
   ...
   6 Sep; assumptions from 7 Sep, flagged — overtaken by the September trough (Kaub 26 cm on
   9 Sep): refresh at the event end with `fetch_kaub.py`; the press-based first version is kept as
