...
| **pipelined flows (22 Sep, user decision after the review of 22 Sep, point OR3)** — `cluster/jobs_20260922_pipe.txt`, two light runs chained on the base (caches rebuild after the capacity rework 2e03f3c) | crude oil and natural gas (B06) reach their buyers without transport: config `pipelined_flows: {products: [B06]}` -> `link.pipelined_share` (1 for a B06 product, the B06 composition share of an import bundle, 0.6-0.97 for the refineries' crude bundles), the pipelined part deposited at the destination node at the supplier's price, the rest routed as before (`send_shipment`), `pipelined_usd` in the routing summary, `pipelined_delivery` in the link table, `--no-pipelines` for the old rule; `2026_pipe_base` and `2018_pipe` (x2 buffer, 20 recovery weeks) paired with `2026_cal_base` and `2018_inv200` | **Read 22 Sep 21:30** (`additional_data/compare_runs_pipe_20260922.csv/.txt`, `matched_estimand_2018_pipe.txt`): losses within 1 % of the twins — 2026 DEU 5,140 mUSD = 0.538 % of a quarter (5,178 = 0.542), peak 1.41 % in week 17; 2018 DEU 3,062 = 0.321 % (3,087 = 0.323), peak 1.22 % in week 21; matched estimand unchanged (1.96 %-months, peak December). The rule changes the exposure, not the loss: 35 % of the dry-bulk bucket's value goes by pipeline, the dry-bulk value blocked at Kaub in the 2026 closure weeks halves (611 / 693 / 778 → 331 / 347 / 369 mUSD/week), liquid and containers unchanged. `2026_pipe_base` and `2018_pipe` are the price-with-closures baselines from now on. Before the rule the audit (`audit_kaub_imports.py`) found 253 mUSD/week of crude imports to pipeline-fed refineries crossing Kaub by barge (a fifth of the value across the gauge, 199 kt/week; KI-45: B06 is typed `mining`, so crude rode as dry bulk). Correction of the audit note: MiRO's crude bundles hold 76-90 days of stock (the `C19: {B06: 90}` override reaches the bundles through their composition), so crude never stopped a refinery in the runs; the correction acts on the baseline Kaub tonnage and on the surcharge pass-through, not on the refinery pathway (refined products, 24-day stocks, real tank-barge cargo). Laptop smoke run on `scenarios/smoke2w.csv` (two steps, every class closed in the second): `mechanism_checks/pipeline_smoke_check.py` |
| **capacity-gate smoke test (22 Sep evening, the review's 'continuous constraint')** — laptop run `smoke_gate10`: `run_rhine.py --profile 2026_first10 --constraint-mode on --closure-threshold -1 --capacities scenarios/rhine_capacities_anchored.csv --edge-capacities ""`, pipeline rule on, ×2 buffer, light export | Kaub's capacity = load factor × its anchored baseline flow every week (−28 % at 92 cm … −91 % at 11 cm), the within-step gate cuts the excess, cut bulk gives up (prohibitive switch) and returns to stock, cut containers go by rail; no closure, no surcharge, rail/road unlimited. `anchor_rhine_capacities.py`: capacity = max(CCNR file, modelled baseline flow) per section (Kaub 148 vs 137 kt/day, Mainz, Koblenz above the file) | **Read 22 Sep 19:00** (`additional_data/gate_smoke_check_20260922.txt`): Kaub shortfall 21 / 16 / 30 / 49 / 47 / 64 / 70 / 78 / 66 / 47 % of the baseline load blocked in weeks 1–10 (re-sent share of the cut a constant 22–23 %, 145–231 kt/week by rail in the trough); German weekly loss 0.1 / 10.8 / 11.8 / 17.4 / 101.7 / 147.0 / 271.7 / 318.8 / 399.6 / 521.9 mUSD against 0 / 0 / 0 / 0 / 0 / 0 / 38.6 / 295.8 / 402.9 / 437.2 for the price-with-closures reference (`2026_cal_base_full`, no pipeline rule): cumulated 1,801 vs 1,175 (+53 %). The gate moves the onset into the surcharge weeks (losses from 55–60 cm, two to three weeks before the first closure) and gives the same order of loss in the closure weeks — the failure mode of the 2018 matched estimand is exactly the onset. Not tested: gate + surcharge (runner change), rail/road capacities (edges need re-naming), the 2018 profile. KI-46 (baseline-capacity warning counts the pipelined tonnage). **Decided 22 Sep: variant B = gate + surcharge** (`run_rhine.py`: the voyage surcharge is also built with the gate and the Kaub edge carries the week's cost shock next to its partial reduction; `--edge-capacities` has no default any more); anchoring = max(CCNR file, modelled flow + 2 %) (`rhine_capacities_anchored.csv`, Kaub 150,500 t/day). Laptop smoke `smoke_gs10` (23 Sep 00:10, `additional_data/gate_surcharge_smoke_check_20260923.txt`, reference `2026_pipe_base`): no cut at t=0; blocked tonnage as the pure gate, re-sent share of the cut down to 12–13 % from week 4 (the surcharge moves the containers to rail before the gate); German loss 0 / 10 / 11 / 15 / 61 / 126 / 235 / 289 / 392 / 380 mUSD in weeks 1–10 vs 0 / 0 / 0 / 0 / 0 / 0 / 39 / 296 / 403 / 437: cumulated 1,519 vs 1,175 (+29 %; pure gate +53 %) — early onset kept, trough weeks slightly below the price variant (the system arrives adapted). Cluster list `cluster/jobs_20260923_gs.txt` (`2026_gs_base` + `2018_gs`, chained: transport and routes rebuild) ready for the user |
